...
class AOC2213:
    def __init__(self, filename):
        ...
        self.pairs = [
            list(map(parse_next_item, pair.split("\n"))) for pair in data.split("\n\n")
        ]

        data = data.replace("\n\n", "\n")
        self.messages = [
            Message(contents=parse_next_item(line)) for line in data.splitlines()
        ]
    ...
    def part_1(self):
        return sum(
            (pair_idx + 1) * self.test(pair) for pair_idx, pair in enumerate(self.pairs)
        )
