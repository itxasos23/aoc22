mod day_08 {
    ...
    fn visible_tree_count(trees: &Vec<Vec<usize>>) -> usize {
        ...
        for (id_row, row) in trees.iter().enumerate() {
            for (id_col, tree) in row.iter().enumerate() {
                ...
                let top_of_tree = &trees[..id_row]
                    .iter()
                    .map(|row| row[id_col])
                    .collect::<Vec<usize>>();
                let bottom_of_tree = &trees[id_row + 1..]
                    .iter()
                    .map(|row| row[id_col])
                    .collect::<Vec<usize>>();

                if left_of_tree.iter().reduce(|acc, x| acc.max(x)).unwrap() < tree
    ...
    fn get_best_scenic_score(trees: &Vec<Vec<usize>>) -> usize {
        ...
        scenic_scores.iter().fold(0, |acc, row| {
            row.iter().fold(0, |acc_r, tree| acc_r.max(*tree)).max(acc)
        })
    }
